...
// Sun shift depending on condition
  const getWeatherImageDay = (condition) => {
  ...
  // Moon shift depending on condition
  const getWeatherImageNight = (condition) => {
    if (condition.toLowerCase().includes("clear")) {
  ...
  const getWeather = async () => {
    ...
    try {
      ...
      const weatherData = {
        dayPhoto: getWeatherImageDay(result.forecast.forecastday[0].day.condition.text),
        nightPhoto: getWeatherImageNight(result.forecast.forecastday[0].day.condition.text),
  ...
  return (
    <div className="relative flex w-screen h-screen align-center justify-center">
      ...
      {/* <div className="flex min-h-screen ">
        <LeftInformation weather={weather} selectedCity={selectedCity} />
        <RightInformation weather={weather} selectedCity={selectedCity} />
      </div> */}
    </div>
      // <LeftCard
      //   weather={weather}
